...
## Тимчасовий пошук гаманців за частковою адресою

`config/partial-wallets.js` — список `{prefix, suffix}` для гаманців, чию повну
адресу ще не знайдено (лише перші 6 і останні 4 символи). Бот звіряє wallet з
КОЖНОГО повідомлення каналу (і create, і finish) проти цих шаблонів. При
першому збігу:
- гаманець одразу трактується як whitelisted (в тому числі для цього ж самого
  повідомлення — якщо це create, позиція відкриється відразу);
- повна адреса пишеться в `discovered_wallets` (SQLite, переживає рестарт);
- в наш notify-канал летить сповіщення `🔍 Знайдено гаманець за частковим збігом`
  з повною адресою.

**Як прибрати рішення, коли всі гаманці знайдені:** перенести знайдені адреси
з повідомлень (або з `SELECT * FROM discovered_wallets`) у `config/wallets.js`,
після чого видалити `config/partial-wallets.js` і виклики `resolveAndTrack` /
блок часткового збігу в `core/wallet-filter.js` (позначено коментарем
"ТИМЧАСОВО"). Таблицю `discovered_wallets` можна лишити — вона більше нізвідки
не читається, якщо `config/partial-wallets.js` видалено.
